...
class Command(django.core.management.base.BaseCommand):
    ...
    def handle2(self, *args, **kwargs):
        ...
        for idx, row in enumerate(appomatic_legacymodels.models.Cogisspill.objects.filter(st_id__gt = src.import_id).order_by("st_id")):
            print "%s @ %s" %(row.facility_id, row.date)
            
            if row.date is None:
                print "IGN"
                continue
            
            operator = appomatic_siteinfo.models.Company.get(row.company_name)
            
            # Format: SS-CCC-NNNNN-XX-XX
            api = row.facility_id.split("-")
            if len(api[0]) != 2:
                api[0:0] = ['47'] # West Virginia is 47...
            while len(api) < 5:
                api.append('00')
            if len(api) != 5 or len(api[0]) != 2 or len(api[1]) != 3 or len(api[2]) != 5 or len(api[3]) != 2 or len(api[4]) != 2:
                print "    Ignoring broken api: %s" % (row.facility_id,)
                continue
            api = '-'.join(api)
            
            lon, lat = pyproj.transform(nad83, wgs84, row.spill_lng, row.spill_lat)

            well = appomatic_siteinfo.models.Well.get(api, None, lat, lon)
            
            info = dict((name, getattr(row, name))
                        for name in appomatic_legacymodels.models.Cogisspill._meta.get_all_field_names())
